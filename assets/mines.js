...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-608fc8bb";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-608fc8bb">'+'<span class="tile-face-608fc8bb tile-front-608fc8bb"></span>'+'<span class="tile-face-608fc8bb tile-back-608fc8bb"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-608fc8bb")){return;}
var back=tile.querySelector(".tile-back-608fc8bb");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-608fc8bb");if(isMine){tile.classList.add("boom-608fc8bb");}
if(dim){tile.classList.add("dimmed-608fc8bb");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-608fc8bb")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-608fc8bb");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-608fc8bb")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-608fc8bb");void panelEl.offsetWidth;panelEl.classList.add("jolt-608fc8bb");}
endRound();}
